...
bool checkIsNotWayland() noexcept
{
...
void printDirs() noexcept
{
	std::cout << "Dirs: user data dir:   " << Config::getDataDir() << '\n';
}

int deviceFloaterMain(int nArgC, char** aArgV) noexcept
{
	...
	const Glib::ustring sAppName = "com.efanomars.device-floater";
	...
	const std::string sError = oWindow.init();
	if (! sError.empty()) {
