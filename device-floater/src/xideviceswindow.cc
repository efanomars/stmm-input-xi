...
static Glib::RefPtr<Gdk::Pixbuf> loadPixBuf(const std::string& sFilePath) noexcept
{
	Glib::RefPtr<Gdk::Pixbuf> refPixbuf;
	try {
		refPixbuf = Gdk::Pixbuf::create_from_file(sFilePath);
	} catch (const Glib::FileError& oErr) {
		std::cout << "Could not find " << sFilePath << '\n';
		std::cout << " -> " << oErr.what() << '\n';
	} catch (const Gdk::PixbufError& oErr) {
		std::cout << "Could not load " << sFilePath << '\n';
		std::cout << " -> " << oErr.what() << '\n';
	}
	return refPixbuf;
}

XiDevicesWindow::XiDevicesWindow(const Glib::ustring sTitle, int32_t nAcquireSeconds) noexcept
: m_oFloater()
...
, m_nAcquiringSecondsLeft(0)
{
	...
	m_sAboutComment = "Copyright © 2016-2020 Stefano Marsili, <stemars@gmx.ch>\n";
	...
	const std::string sIsFloatingFile = Config::getDataDir() + "/images/";
	m_refPixbufFloating = loadPixBuf(sIsFloatingFile + "buoy_32313.png");
	m_refPixbufKeyboard = loadPixBuf(sIsFloatingFile + "keyboard_1420.png");
	m_refPixbufPointer = loadPixBuf(sIsFloatingFile + "mouse_347.png");

	Gtk::Window::add(m_oMainVBox);
	show_all_children();
}
std::string XiDevicesWindow::init() noexcept
{
	const std::string sError = m_oFloater.init(get_display());
	if (! sError.empty()) {
		return sError; //-------------------------------------------------------
	}
	// successful
...
void XiDevicesWindow::recreateList() noexcept
{
...
//std::cout << "XiDevicesWindow::recreateList()  aDevices.size()=" << aDevices.size() << std::endl;
	...
	for (auto& oData : aDevices) {
		...
		switch (oData.m_eType) {
			case DeviceFloater::DEVICE_TYPE_FLOATING: { refPixbuf = m_refPixbufFloating; } break;
			case DeviceFloater::DEVICE_TYPE_KEYBOARD: { refPixbuf = m_refPixbufKeyboard; } break;
			case DeviceFloater::DEVICE_TYPE_POINTER: { refPixbuf = m_refPixbufPointer; } break;
			default: assert(false);
		}
		if (refPixbuf) {
			(*itListDevice)[m_oDeviceColumns.m_oColIsFloating] = std::move(refPixbuf);
		}
...
void XiDevicesWindow::devicesChanged() noexcept
{
//std::cout << "XiDevicesWindow::devicedChanged()" << std::endl;
	recreateList();
}
void XiDevicesWindow::slaveAdded(const DeviceFloater::SlaveDeviceData& oData) noexcept
{
...
void XiDevicesWindow::onAcquireStartClicked() noexcept
{
	if (m_bAcquiring) {
...
bool XiDevicesWindow::onAcquiringTimeout() noexcept
{
...
void XiDevicesWindow::onAboutClicked() noexcept
{
	const bool bActive = m_oAboutButton.get_active();
	if (bActive) {
