...
def main():
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f             {}/include/stmm-input-gtk-xi".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-gtk-xi.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-gtk-xi.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f       {}/lib/pkgconfig/stmm-input-gtk-xi.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f        {}/share/doc/libstmm-input-gtk-xi".format(sSudo, sInstallDir).split())
		if sInstallDir == "/usr":
			subprocess.check_call("{} rm  -f   /etc/stmm-input-dl/stmm-input-gtk-xi.dlp".format(sSudo).split())
		else:
			subprocess.check_call("{} rm  -f {}/etc/stmm-input-dl/stmm-input-gtk-xi.dlp".format(sSudo, sInstallDir).split())
