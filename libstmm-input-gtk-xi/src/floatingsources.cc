...
XIEventSource::XIEventSource(Gdk::Display* p0Display, int nXiOpcode)
: Glib::Source()
, m_p0XDisplay(None)
...
XIEventSource::~XIEventSource()
{
//std::cout << "XIEventSource::~XIEventSource()" << std::endl;
}
sigc::connection XIEventSource::connect(const sigc::slot<bool, XIDeviceEvent*>& oSlot)
{
	return connect_generic(oSlot);
}

bool XIEventSource::prepare(int& nTimeout)
{
	if (m_bSkipNextEvent) {
...
bool XIEventSource::checkEvent(XEvent& oXEvent, bool& bIsGenericEvent, bool& bIsForFloating, bool& bIsFloatingEnterFocus)
{
...
bool XIEventSource::dispatch(sigc::slot_base* p0Slot)
{
	bool bContinue = true;
