...
void GtkWindowData::enable(const shared_ptr<GtkAccessor>& refAccessor, FloGtkDeviceManager* p0Owner) noexcept
{
	assert(refAccessor);
...
void GtkWindowData::setXIEventsForAllDevices(bool bSet) noexcept
{
...
bool GtkWindowData::setXIEventsForDevice(int32_t nXDeviceId, bool bSet) noexcept
{
//std::cout << "Flo::GtkWindowData::selectXIEvents()  nXDeviceId=" << nXDeviceId << "  nXWinId=" << m_nXWinId << "  bSet=" << bSet << '\n';
	assert(m_p0Owner != nullptr);
	//assert(m_bIsEnabled);
	...
	if (!m_bIsRealized) {
		...
		m_bIsRealized = true;
//std::cout << "Flo::GtkWindowData::selectXIEvents()  just realized!" << '\n';
