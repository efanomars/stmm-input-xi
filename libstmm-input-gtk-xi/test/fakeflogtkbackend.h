...
{
public:
	FakeGtkBackend(::stmi::FloGtkDeviceManager* p0Owner, const Glib::RefPtr<Gdk::Display>& refGdkDisplay) noexcept;
	std::string getDeviceName(int32_t nXDeviceId) const noexcept override
	{
		#ifndef NDEBUG
		const int32_t nIdx = 
		#endif
	...
	const std::vector< int32_t >& getXDeviceIds() noexcept override
	{
		return m_aXDeviceIds;
	}
	void focusDevicesToWindow(const std::shared_ptr<Private::Flo::GtkWindowData>& refWinData) noexcept override;
	//
	...
	void simulateRemoveDevice(int32_t nXDeviceId) noexcept
	{
	...
	void simulateDeviceChanged(int32_t nXDeviceId) noexcept
	{
		#ifndef NDEBUG
		const int32_t nIdx = 
		#endif
	...
	void simulateKeyEvent(int32_t nXDeviceId, ::Window nWinId, bool bPress, int32_t nHardwareCode) noexcept;
private:
	int32_t findXDeviceId(int32_t nXDeviceId) const noexcept
	{
