...
{
public:
	explicit FakeGtkWindowData(FakeGtkBackend* p0FakeGtkBackend) noexcept
	...
	void enable(const shared_ptr<GtkAccessor>& refAccessor, FloGtkDeviceManager* p0Owner) noexcept override;
	...
	void connectAllDevices() noexcept override
	{
		if (!m_bIsRealized) {
			return;
		}
	...
	void connectDevice(int32_t nXDeviceId) noexcept override
	{
		if (!m_bIsRealized) {
			return;
		}
	...
	::Window getXWindow() const noexcept override
	{
		if (m_bIsRealized) {
			return m_nXWinId;
		} else {
	...
	void simulateWindowRealized() noexcept
	{
		if (!m_bIsRealized) {
...
private:
	static ::Window getNewWinId() noexcept
	{
...
//std::cout << "GtkWindowData::disconnect()" << std::endl;
	...
	void connectDeviceToWindow(int32_t nXDeviceId) noexcept
	{
...
{
public:
	explicit FakeGtkWindowDataFactory(FakeGtkBackend* p0FakeGtkBackend) noexcept
	: Private::Flo::GtkWindowDataFactory()
	, m_p0FakeGtkBackend(p0FakeGtkBackend)
	{
		assert(p0FakeGtkBackend != nullptr);
	}
	std::shared_ptr<Private::Flo::GtkWindowData> create() noexcept override
	{
	...
	void simulateWindowSetRealizedWhenEnabled(bool bRealizedOnEnable) noexcept
	{
